...
router.post("/member",async (ctx)=>{
     ...
     member.save((errorInDbTransaction:Error)=>{
...
router.get("/member",async (ctx)=>{
    try {
       await ChurchMember.find((error:Error,members:Document)=>{
           if(error){
               console.error("The Following Error Occured While Try To Get Data: ",error)
           }else{
